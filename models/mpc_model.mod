...
param Occupancy {1..total_rooms, 1..duration};		# Occupancy in the Room
...
# Variables
var T {1..total_rooms, 1..duration} >= 0;					# Room Temperature
var Delta_T_SPOT {1..total_rooms, 1..duration+1} >= 0;		# Change in Temperature of SPOT Region
var T_SPOT {1..total_rooms, 1..duration} >= 0;				# Temperature in SPOT Region
var Delta_T_NoSPOT {1..total_rooms, 1..duration+1} >= 0;	# Change in Temperature of No SPOT Region
var T_NoSPOT {1..total_rooms, 1..duration+1} >= 0;			# Temperature in No-SPOT Region
...
var SPOT_Status {1..total_rooms, 1..duration} >= 0;			# SPOT State
var Ratio {1..duration} >= 0;								# Mixing Ratio
var PMV {1..total_rooms, 2..duration+1};					# PMV
var Fan_Speed {1..total_rooms, 1..duration} >= 0;			# Fan Speed
var a10 >= 0;
...
minimize total_energy : sum {t in 1..duration} (
						...
						sum {k in 1..total_rooms, t in 1..duration} (
							( Coefficient_SPOT_Power * SPOT_Status[k, t] )
						);

# Constraints
subject to Initialize_Delta_Temperature_In_SPOT_Region {k in 1..total_rooms}: Delta_T_SPOT[k, 1] = Delta_T_SPOT_Init[k];
subject to Delta_Temperature_In_SPOT_Region {k in 1..total_rooms, t in 1..duration}:
		Delta_T_SPOT[k, t+1] = ( CoRC_CiRT * Delta_T_SPOT[k, t] ) + ( CoSI_SCS * SPOT_Status[k, t] ) + 
						( CoOI_OHL * Occupancy[k, t] );
...
subject to PMV_Constraint {k in 1..total_rooms, t in 2..duration+1}:
		PMV[k, t] = Occupancy[k, t-1] * ( 0.2466 * ( T_NoSPOT[k, t] + Delta_T_SPOT[k, t] - 
		CoRC_CiR1T * Delta_T_SPOT[k, t-1] ) + ( 0.5810 * Fan_Speed[k, t-1] * Fan_Speed[k, t-1] ) - 
		( 1.4075 * Fan_Speed[k, t-1] ) - 5.4668);
